...
%Implementing mixed boundary conditions
...
%Mixed Boundary conditions (nan) if not specified.
%Input in the form of list 
%Eg. a*u(0)+b*u'(0)=g(0) Input as 'Ma=[g(0)/b, -a/b]'
ua = nan;
ub = exp(2);
Ma = [exp(1),-1];
Mb = nan;
...
syms y(x);
Dy = diff(y);
a(x) = dsolve( p(x)*diff(y,x,x)+q(x)*diff(y,x)+r(x)*y-f(x)==0, y(1)+Dy(1)==exp(1), y(exp(1))==exp(2));
...
m2 = diag(mid,0);
K = m1+m2+m3;
%Change K according to the boundary conditions
if(~isnan(Ma))
    K(1,1) = K(1,1)-Ma(2);
end
if(~isnan(Mb))
    K(n+1,n+1) = K(n+1,n+1)+Mb(2);
end
